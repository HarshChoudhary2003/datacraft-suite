...
/** Resolve a palette by name, falling back to the default when unknown/blank. */
export function resolvePalette(name?: string | null): string[] {
  if (name && THEMES[name]) return THEMES[name];
  return THEMES[DEFAULT_THEME];
}

/** Deterministic colour lookup that wraps around the palette length. */
...
/** Mix a colour with transparency; works for hex, oklch and var() references. */
export function withAlpha(color: string, alpha: number): string {
  const pct = Math.max(0, Math.min(100, Math.round(alpha * 100)));
  return `color-mix(in srgb, ${color} ${pct}%, transparent)`;
}

/** Neutral wash used to de-emphasise unselected / unhovered series. */
export function dimmedColor(color: string): string {
  return `color-mix(in srgb, ${color} 22%, var(--muted))`;
}

/** Lighter/darker sibling of a colour, for gradient stops and hover accents. */
export function shiftColor(color: string, towards: "light" | "dark", amount = 0.25): string {
  ...
  const target = towards === "light" ? "white" : "black";
...
/**
 * Gradient stop presets shared by area fills, bar fills and KPI sparklines so
 * every surface renders the same vertical light-to-transparent polish.
 */
export function gradientStops(
  color: string,
  variant: "area" | "bar" | "spark" = "area",
): GradientStop[] {
  if (variant === "bar")
    return [
...
/** Stable gradient element id for a widget/series pair. */
...
export const tooltipStyle = {
  ...
  borderRadius: 14,
  boxShadow:
    "0 18px 48px -12px rgba(0, 0, 0, 0.35), inset 0 1px 1px color-mix(in srgb, white 12%, transparent)",
  fontSize: 12,
  padding: "10px 12px",
};
